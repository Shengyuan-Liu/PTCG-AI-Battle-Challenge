...
### 9.6 提交踩坑与诊断（2026-06-27，实战）

**坑 1（致命）：`kaggle_environments` 用 `exec()` 跑 `main.py`，上下文里没有 `__file__`。**
顶层 `main.py` 里任何 `__file__`（如 `os.path.dirname(__file__)`）→ `NameError` → 整个 agent 加载失败 → 提交 `SubmissionStatus.ERROR`。
- 现象极具误导性：v1(MCTS)、v2(纯NN) 都 ERROR，v3(rule, 无 `__file__`) COMPLETE → 一度误判为「torch 跑不了」。**其实和 torch 无关。**
- 修复：`try: _HERE = os.path.dirname(os.path.abspath(__file__)) except NameError: _HERE = "/kaggle_simulations/agent"`。被 import 的子模块（如 `model/agent.py`）有 `__file__`，但顶层 `main.py` 一定要容错。

**坑 2：rule 兜底救不了「加载期 / 进程级」崩溃。** `try/except` 只能接 Python 异常；`main.py` 模块加载失败（NameError）或进程被 SIGKILL（OOM）时，兜底逻辑根本没机会执行。

**诊断方法（拿真实 traceback，CLI 只给 "Validation Episode failed."）：**
```bash
kaggle competitions episodes <submission_id>      # 列出该提交的验证 episode
kaggle competitions logs <episode_id> 0           # 0=我方 agent，stderr 里有真实 traceback
```
也可用 Python API 取 `_error_description`：`api.competition_submissions("pokemon-tcg-ai-battle")`。

**提交清单（每次提交前自检）：**
- [ ] 顶层 `main.py` 不直接用 `__file__`（已容错）。
- [ ] 本地用「exec 无 `__file__`」复现：`exec(compile(open("main.py").read(),"main.py","exec"), {})` 跑通。
- [ ] tar 顶层有 `main.py`（`tar -tzf` 检查）、含 `cg/libcg.so`、`deck.csv`、权重。
- [ ] 选卡组阶段返回恰好 60 个 card ID。
- [ ] 模型失败有 rule 兜底（防可捕获异常，但救不了加载期崩溃）。

### 9.7 放大网络（去掉 MCTS 后的关键解锁，2026-06-27）

**洞察**：原本「网络必须小」是因为 MCTS 每步调网络几百次（CPU+10min 会超时）。改成**纯 NN 单次前向**部署后，这个约束消失 → 可以换更复杂的 Transformer。

- 架构从 `(128,2,256,1,1)≈12M` → **`(256,4,1024,3,3)≈29M`**（`network.ARCH`，可用 `PTCG_DMODEL` 等环境变量覆盖以扫超参）。
- CPU 单次前向仅 **5ms/步**（200 步/局 ≈ 1s，远低于 10min）；权重 117MB，tar 105MB（Kaggle 接受）。
- **判决（纯 NN，对 rule，各 200 局）**：大模型 **60%** vs 小模型 v4 **52%** → 大模型明显更强（约 2+σ）。
- 训练：32 局 replay（SL 样本 613→1135）+ AZ self-play（best 69% vs rule@iter6）。**注意 self-play 自造数据是主训练信号，不受 replay 数限制**——这让大模型在数据仍偏少时也能训起来。

**已部署**：`model/out/model_final.pth` = 大模型；提交 v5 COMPLETE 上天梯。下一步靠**更多 replay**（dataset API 列文件接口当日限流，待重置）继续把数据/模型做大。
